...
const NavbarTheme = ({ color }: { color: string }) => {
  const { isTrue: isOpen, toggle } = useToggle()

  return (
    <nav className={`navbar navbar-expand-lg navbar-dark bg-${color} mb-2`}>
      <Container fluid>
...
const BasicNavbar = () => {
  const { isTrue: isOpen, toggle } = useToggle()
  return (
    <ComponentContainerCard title="Basic Navbar">
...
const NavbarWithBrandImage = () => {
  const { isTrue: isOpen, toggle } = useToggle()
  const { isTrue: isNavbarOpen, toggle: toggleNavbar } = useToggle()

  return (
    <ComponentContainerCard title="Navbar With Brand Image">
      <nav className="navbar navbar-expand-lg navbar-light bg-light">
        <div className="container-fluid">
          <Link className="navbar-brand" href="">
            <Image src={logoSm} height={26} alt="logo-sm" className="me-1" />
          </Link>
          <button onClick={toggle} className="navbar-toggler" type="button">
            <span className="navbar-toggler-icon" />
          </button>
      ...
      <nav className="navbar navbar-expand-lg navbar-light bg-light mt-2">
        <Container fluid>
          <Link className="navbar-brand" href="">
            <Image src={logoSm} height={26} alt="logo-2" className="me-2" />
            <Image src={logoDark} height={16} alt="logo-dark" />
          </Link>
          <button
...
const NavbarThemeColors = () => {
  const navbarColors: string[] = ['primary', 'secondary', 'dark']

  return (
    <ComponentContainerCard title="Theme Colors">
      {navbarColors.map((color, idx) => (
        <NavbarTheme color={color} key={idx} />
      ))}
    </ComponentContainerCard>
  )
}
