...
const ProjectCard = ({ project }: { project: ProjectType }) => {
  const { budget, client, deadlineDate, description, logo, name, progress, startDate, status, tasks, teams } = project
  return (
    <Card>
      <CardBody>
        <div className="position-absolute  end-0 me-3">
          <span
            className={clsx(
              'badge rounded bg-transparent border ms-1 p-1',
              status === 'In Progress' ? 'border-warning text-warning' : 'border-primary text-primary',
            )}>
            {status}
          </span>
        </div>
        <div className="text-center border-dashed-bottom pb-3">
          <Image src={logo} alt="logo" height={80} className="rounded-circle d-inline-block" />
        ...
        <div className="d-flex justify-content-between fw-semibold align-items-center  mt-3">
          <div className="img-group d-flex justify-content-center">
            {teams?.members &&
              teams?.members.slice(0, 3).map((member, idx) => (
                <span role="button" className={clsx('user-avatar position-relative d-inline-block', { 'ms-n2': idx != 0 })} key={idx}>
                  {member?.avatar && <Image src={member?.avatar} alt="avatar" className="thumb-md shadow-sm rounded-circle" />}
                </span>
              ))}
            {teams?.members && (
