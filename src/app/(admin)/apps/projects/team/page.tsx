...
import type { TeamType } from '@/types/data'

export const metadata: Metadata = { title: 'Team' }

const TeamCard = ({ description, logo, progress, teamName, members, user }: TeamType) => {
  return (
    <Card>
      <CardBody>
        <div className="text-center border-dashed-bottom pb-3">
          <Image src={logo} alt="team-logo" height={80} className="rounded-circle d-inline-block" />
          ...
          <div className="img-group d-flex justify-content-center mt-3">
            {members?.slice(0, 4).map((member, idx) => (
              <Link className="user-avatar position-relative d-inline-block ms-n2" href="" key={idx}>
                {member && <Image src={member?.avatar} alt="avatar" className="thumb-md shadow-sm rounded-circle" />}
              </Link>
            ))}
            {members && (
        ...
        <Row className="mt-3 align-items-center">
          <Col xs="auto" md={6}>
            <div className="d-flex align-items-center">
              {user?.avatar && <Image src={user?.avatar} className="me-2 align-self-center thumb-lg rounded" alt="..." />}
        ...
        <div className="mt-3 text-center">
          <Link href="/apps/chat" className="btn btn-outline-primary px-2 d-inline-flex align-items-center">
