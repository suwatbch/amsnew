...
const Board = () => {
  const { onDragEnd, sections, getAllTasksPerSection, newTaskModal, sectionModal, sectionForm } = useKanbanContext()
          ...
          <div className="kanban-board">
            {sections.map((section) => (
              <Droppable key={section.id} droppableId={section.id}>
                {(provided) => (
                  <div ref={provided.innerRef} className="kanban-col" {...provided.droppableProps}>
                    <div className="my-3">
                      <div
                        className={clsx(
                          'd-flex justify-content-between align-items-center border-bottom border-2',
                          'border-' + getKanbanSectionVariant(section.title),
                        )}>
                        <div>
                    ...
                    <SimplebarReactClient style={{ height: 'calc(100vh - 100px)' }} className="pt-1">
