...
const TaskItem = ({
  task: { priority, commentsCount, completedTasks, title, totalTasks, description, image, tags, sectionId, id },
}: {
  task: KanbanTaskType
}) => {
  ...
  return (
    <CardBody>
      ...
      <span
        className={clsx(
          'badge rounded fw-normal px-1 mb-1',
          `text-${getKanbanTaskPriorityVariant(priority)} bg-${getKanbanTaskPriorityVariant(priority)}-subtle`,
        )}>
        {priority}
      </span>
      ...
      {image && (
        <p className="p-3 rounded-md">
          <Image src={image} alt="illustration" className="img-fluid mx-auto" />
        </p>
      )}
      ...
      {tags && (
        <div>
          {tags.map((tag, idx) => {
            ...
            return (
              <span key={idx} className={clsx('badge rounded fw-normal px-1 mb-1 me-1', `text-${variant} bg-${variant}-subtle`)}>
                {tag}
              </span>
            )
          })}
      ...
      {percentageCompleted > 0 && (
        <>
          ...
          <ProgressBar className="bg-secondary-subtle" style={{ height: 5 }} now={percentageCompleted}>
        ...
        <Col className="align-self-center">
          <div className="img-group d-flex justify-content-center">
            <span className="user-avatar position-relative d-inline-block">
              <Image src={avatar1} alt={'avatar1'} className="thumb-sm shadow-sm rounded-circle" />
            </span>
            <span className="user-avatar position-relative d-inline-block ms-n2">
              <Image src={avatar2} alt={'avatar2'} className="thumb-sm shadow-sm rounded-circle" />
            </span>
            <span className="user-avatar position-relative d-inline-block ms-n2">
              <Image src={avatar6} alt={'avatar6'} className="thumb-sm shadow-sm rounded-circle" />
            </span>
          </div>
        </Col>
      </Row>
