...
import { Accordion, AccordionBody, AccordionButton, AccordionHeader, AccordionItem, Col, ProgressBar, Row } from 'react-bootstrap'

const TaskDetails = async () => {
  const tasks = await getAllTasks()
  return (
    <ComponentContainerCard title="Tasks & Details">
      <Accordion defaultActiveKey="0" className="card-bg " id="task-1">
        {tasks.map((task, idx) => (
          <AccordionItem as="div" key={idx} eventKey={`${idx}`} className="task-accordion rounded  border-dashed border-theme-color mb-3 p-2">
            <AccordionHeader>
                    ...
                    <tr>
                      ...
                      <td className="text-end">
                        ...
                        {task.projects && (
                          <span
                            className={clsx(
                              'badge rounded bg-transparent border p-1',
                              task.projects.status === 'In Progress' ? 'border-blue text-blue' : 'border-info text-info',
                            )}>
            ...
            <AccordionBody className="bg-body border-dashed-top  p-3 p-3">
              ...
              <Row className="row-cols-auto g-3">
                <Col md={3}>
                  <div className="d-flex align-items-center">
                    {task.allUsers && <Image src={task.allUsers?.avatar} className="thumb-md align-self-center rounded-circle" alt="..." />}
                ...
                <Col md={2} className="text-end align-self-center">
                  ...
                  <ProgressBar now={task.projects?.progress} variant="secondary" style={{ height: 5 }} />
                </Col>
                <Col md={2}>
