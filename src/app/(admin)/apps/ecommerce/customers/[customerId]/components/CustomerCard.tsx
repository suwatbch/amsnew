...
const CustomerCard = ({ customer }: { customer: CustomerType }) => {
  const { name, avatar, email } = customer
          ...
          <Col>
            <div className="d-flex align-items-center">
              <div className="position-relative">
                <Image src={avatar} alt="avatar" className="rounded-circle img-fluid" />
                <div className="position-absolute top-50 start-100 translate-middle">
                  <Image src={bahaFlag} alt="baha-flag" className="rounded-circle thumb-sm border border-3 border-white" />
                </div>
              </div>
