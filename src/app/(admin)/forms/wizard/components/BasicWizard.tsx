  ...
  return (
    <div className="tab-pane" id="step4">
      ...
      <div className="form-check my-4 text-center">
        <Image src={taskImg} className="mb-3" height={60} alt="task" />
...
const BasicWizard = () => {
  const NavButtons: React.FC<any> = (props: StepWizardChildProps) => {
    ...
    return (
      <nav>
        <div className="nav nav-tabs" id="nav-tab">
          <Link href="" className={`nav-link ${props.currentStep == 1 && 'active'}`} id="step1-tab" onClick={() => handleStep(1)}>
            Seller Details
          </Link>
          <Link href="" className={`nav-link ${props.currentStep == 2 && 'active'}`} id="step2-tab" onClick={() => handleStep(2)}>
            Company Document
          </Link>
          <Link href="" className={`nav-link ${props.currentStep == 3 && 'active'}`} id="step3-tab" onClick={() => handleStep(3)}>
            Bank Details
          </Link>
          <Link href="" className={`nav-link ${props.currentStep == 4 && 'active'}`} id="step4-tab" onClick={() => handleStep(4)}>
            Confirm Detail
          </Link>
        </div>
      </nav>
  ...
  const ActionButtons = (props: StepWizardChildProps) => {
    ...
    return (
      <div>
        {props.currentStep > 1 && (
          <Button type="button" id="stepPrev" variant="secondary" className="float-start" onClick={handleBack}>
            Previous
          </Button>
        )}

        {props.currentStep < props.totalSteps && (
          <Button type="button" id="stepNext" variant="primary" onClick={handleNext} className="float-end">
            Next
          </Button>
        )}

        {props.currentStep === props.totalSteps && (
          <Button type="button" id="stepFinish" variant="danger" onClick={handleFinish} className="float-end">
            Finish
          </Button>
        )}
      </div>
