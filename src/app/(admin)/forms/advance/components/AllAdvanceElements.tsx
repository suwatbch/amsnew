...
const ReactSelectElement = () => {
  return (
    <ComponentContainerCard title="Select Components">
      <Row className="g-2">
        <Col md={4}>
...
const ReactColorElement = () => {
  ...
  const [displaySketch, setDisplaySketch] = useState<boolean>(false)
  const [displaySketch1, setDisplaySketch1] = useState<boolean>(false)
  const [displaySketch2, setDisplaySketch2] = useState<boolean>(false)
  ...
  const onSwatchHoverSketch = (color: any) => {
    const format = `rgba(${color.rgb.r}, ${color.rgb.g}, ${color.rgb.b}, ${color.rgb.a})`
    setColorSketch(format)
  }

  const onSwatchHoverSketch1 = (color: any) => {
    const format = `rgba(${color.rgb.r}, ${color.rgb.g}, ${color.rgb.b}, ${color.rgb.a})`
    setColorSketch1(format)
  }

  const onSwatchHoverSketch2 = (color: any) => {
    const format = `rgba(${color.rgb.r}, ${color.rgb.g}, ${color.rgb.b}, ${color.rgb.a})`
    setColorSketch2(format)
  }

  return (
    <ComponentContainerCard title="React Color">
      <Row className="g-2">
        <Col md={4}>
          <input
            onClick={handleSketch}
            className="form-control color-input"
            style={{ background: colorSketch }}
            value={colorSketch}
            defaultValue="#22c55e"
          />
          {displaySketch ? (
        ...
        <Col md={4}>
          <input onClick={handleSketch1} className="form-control " value={colorSketch1} style={{ background: colorSketch1 }} />
          {displaySketch1 ? (
            <div className="m-2">
        ...
        <Col md={4}>
          <input onClick={handleSketch2} className="form-control " value={colorSketch2} style={{ background: colorSketch2 }} />
          {displaySketch2 ? (
            <div className="m-2">
...
const CustomTouchSpinElement = () => {
  ...
  return (
    <ComponentContainerCard title="Custom Touch Spin">
...
const ReactFlatpickerElement = () => {
  return (
    <ComponentContainerCard title="React Flatpicker">
      <Row>
        <Col md={12}>
          <label className="mb-2">Default Datepicker</label>
          <Flatpickr className="form-control mb-3" options={{ dateFormat: 'd M, Y' }} />
          <label className="mb-2">Date Range Picker</label>
          <div className="input-group" id="DateRange">
            <Flatpickr className="form-control" options={{ dateFormat: 'd M, Y', mode: 'range' }} />
          </div>
        </Col>
        <Col md={6}>
    ...
    <ComponentContainerCard title="Mask">
      ...
      <div className="mb-2">
        ...
        <MaskedInput
          ...
          mask={[/[0-3]/, /[0-9]/, '.', /[0-1]/, /[1-9]/, '.', /[1-2]/, /\d/, /\d/, /\d/]}
        />
      </div>
      <div className="mb-2">
      ...
      <div>
        ...
        <MaskedInput
          ...
          mask={[/[1-2]/, /\d/, /\d/, /\d/, '/', /[0-1]/, /[1-9]/, '/', /[1-2]/, /\d/, ' ', /[0-2]/, /\d/, ':', /[0-5]/, /\d/]}
        />
      </div>
    </ComponentContainerCard>
  )
